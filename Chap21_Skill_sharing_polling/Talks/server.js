...
router.add("GET",talkPath,async (server,title)=>{
    if(title in server.talks){
        ...
        return {status:4040,body:`No Talk '${title}' found`};
    }
})
...
router.add("POST",/^\/talks\/([^\/]+)\/comments$/,async (server,title,request)=>{
    ...
    }catch (e){return {status:400,body:"Invalid JSON"}}
    ...
    if(!comment||typeof comment.author!="string" || typeof comment.message !="string"){
        ...
        return {status:400,body:`No talk '${title}' found`}
    }
...
new SkillShareServer(Object.create(null)).start(9000)
